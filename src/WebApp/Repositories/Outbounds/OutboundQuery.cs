...
{
/// <summary>
/// File: OutboundQuery.cs
/// Purpose: easyui datagrid filter query 
/// Created Date: 2020/9/3 10:27:14
/// Author: neo.zhu
/// Tools: SmartCode MVC5 Scaffolder for Visual Studio 2017
/// Copyright (c) 2012-2018 All Rights Reserved
/// </summary>
   public class OutboundQuery:QueryObject<Outbound>
   {
		public OutboundQuery Withfilter(IEnumerable<filterRule> filters)
        {
           if (filters != null)
           {
               foreach (var rule in filters)
               {
						...
						if (rule.field == "StockQty" && !string.IsNullOrEmpty(rule.value) && rule.value.IsDecimal())
						{
							var val = Convert.ToDecimal(rule.value);
							switch (rule.op) {
                            case "equal":
