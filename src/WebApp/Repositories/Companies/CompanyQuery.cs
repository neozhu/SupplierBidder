...
namespace WebApp.Repositories
{
  public class CompanyQuery : QueryObject<Company>
  {
    ...
    public CompanyQuery Withfilter(IEnumerable<filterRule> filters)
    {
      if (filters != null)
      {
        foreach (var rule in filters)
        {
          ...
          {
            this.And(x => x.Scope.Contains(rule.value));
          }

          if (rule.field == "Category" && !string.IsNullOrEmpty(rule.value))
          {
            this.And(x => x.Category.Contains(rule.value));
          }
    ...
    public CompanyQuery WithfilterSupplier(IEnumerable<filterRule> filters)
    {
