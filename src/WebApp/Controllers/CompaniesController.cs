...
namespace WebApp.Controllers
{
  ...
  public class CompaniesController : Controller
  {
    ...
    [HttpGet]
    public async Task<JsonResult> GetComboData(string q = "", int page = 1, int rows = 100, string sort = "Id", string order = "asc")
    {
    ...
    public async Task<JsonResult> GetSuppliers(string q = "") {
      var data =await this.companyService.Queryable()
        .Where(x => x.Type == "供应商" && x.Name.Contains(q))
        .Select(x=>new { x.Id,x.Name,x.Scope,x.Category})
        .ToListAsync();
    ...
    [HttpGet]
    //[OutputCache(Duration = 10, VaryByParam = "*")]
    public async Task<JsonResult> GetSupplierData(int page = 1, int rows = 10, string sort = "Id", string order = "asc", string filterRules = "")
    {
      var filters = JsonConvert.DeserializeObject<IEnumerable<filterRule>>(filterRules);
      var pagerows = ( await this.companyService
                           .Query(new CompanyQuery().WithfilterSupplier(filters))
                         .OrderBy(n => n.OrderBy(sort, order))
                         .SelectPageAsync(page, rows, out var totalCount) )
                                       .Select(n => new
                                       {
